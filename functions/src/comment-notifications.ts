...
export const onCommentCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    // Get companyId from submission document
    let companyId = "";
    ...
    try {
      ...
      if (mentionedUserIds.length > 0) {
        ...
        await batch.commit();
        console.log(
          `Created notifications for ${mentionedUserIds.length} users`,
        );
      }
    ...
    try {
      ...
      for (const companyDoc of companiesSnap.docs) {
        ...
        if (notificationsSnap.size > 0) {
          await batch.commit();
          console.log(
            `Deleted ${notificationsSnap.size} old notifications from company ${companyDoc.id}`,
          );
        }
      }
