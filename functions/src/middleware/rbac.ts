...
const ROLE_PERMISSIONS: Record<UserRole, Set<string>> = {
  ...
  viewer: new Set(["submissions:read", "analytics:read"]),
};
...
export async function verifyRBAC(
  ...
  try {
    ...
    req.user = {
      ...
      companyId: (userData as Record<string, unknown>).companyId as string,
      role: (userData as Record<string, unknown>).role as UserRole,
    };
