...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { submissionIds, status } = req.body as BulkOperationPayload & { status: string };
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { submissionIds, priority } = req.body as BulkOperationPayload & { priority: string };
      const companyId = req.companyId;
      const userId = req.userId;
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { submissionIds, category } = req.body as BulkOperationPayload & { category: string };
      const companyId = req.companyId;
      const userId = req.userId;
      ...
      if (operation.previousValues && Array.isArray(operation.previousValues)) {
        ...
        for (let i = 0; i < operation.previousValues.length; i += BATCH_SIZE) {
          ...
          for (const item of batchData) {
            const submissionRef = db.collection('submissions').doc(item.submissionId);
            batch.update(submissionRef, {
              ...item.previousData,
              updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            });
        ...
        await db.collection('undoOperations').doc(undoOperationId).set(undoOperation);
        ...
        res.status(400).json({ error: 'No previous values stored for this operation' });
      }
    } catch (error) {
