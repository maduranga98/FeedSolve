...
// Get team members
router.get('/:companyId/members', requirePermission('team:read'), async (req: PermissionRequest, res) => {
  try {
...
// Update member role
router.put('/:companyId/members/:userId/role', requirePermission('team:manage'), async (req: PermissionRequest, res) => {
  try {
    ...
    const targetData = targetUser.data() as any;
    const currentRole = targetData.role;

    // Check if user can manage this role
    if (!canManageRole(req.user.role, currentRole)) {
...
// Remove team member
router.delete('/:companyId/members/:userId', requirePermission('team:manage'), async (req: PermissionRequest, res) => {
  try {
    const { companyId, userId } = req.params;
    ...
    if (!canManageRole(req.user.role, userData.role)) {
...
// Get audit logs
router.get('/:companyId/audit-logs', requirePermission('audit:read'), async (req: PermissionRequest, res) => {
  try {
    const { companyId } = req.params;
