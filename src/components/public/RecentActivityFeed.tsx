...
export function RecentActivityFeed({ submissions, formatResolutionTime }: RecentActivityFeedProps) {
  if (submissions.length === 0) {
    return (
      <div className="rounded-xl border border-[#d6cabf] bg-white p-6 text-center text-sm text-[#78716c]">
  ...
  return (
    <div className="rounded-xl border border-[#d6cabf] bg-white p-5 shadow-sm">
      <div className="mb-5 rounded-lg bg-[#f5f0ec] px-4 py-3 text-sm text-[#78716c]">
        ...
        {submissions.map((submission) => (
          <li key={submission.id} className="relative flex gap-3">
            ...
            <div className="min-w-0 flex-1 border-b border-[#EEF2F5] pb-4 last:border-b-0 last:pb-0">
              ...
              <p className="mt-1 text-xs uppercase tracking-wide text-[#78716c]">Anonymized public update</p>
            </div>
          </li>
        ))}
      </ol>
